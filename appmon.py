...
def update_k8s_data():
    ...
    while True:
        ...
        time.sleep(300)"""
            ...
            # Obtener información de los pods en todos los namespaces
            pods_data = run_kubectl_command("get pods --all-namespaces")
            k8s_data["pods"] = pods_data if "error" not in pods_data else {"items": [], "error": pods_data["error"]}

            # Obtener información de los servicios en todos los namespaces
            services_data = run_kubectl_command("get services --all-namespaces")
            k8s_data["services"] = services_data if "error" not in services_data else {"items": [], "error": services_data["error"]}

            # Obtener información de los deployments en todos los namespaces
            deployments_data = run_kubectl_command("get deployments --all-namespaces")
            k8s_data["deployments"] = deployments_data if "error" not in deployments_data else {"items": [], "error": deployments_data["error"]}

            # Registrar la última actualización
            k8s_data["last_updated"] = time.strftime("%Y-%m-%d %H:%M:%S")
            logging.info(f"Datos de Kubernetes actualizados a las {k8s_data['last_updated']}")
        except Exception as e:
            logging.error(f"Error general al actualizar datos: {str(e)}")
